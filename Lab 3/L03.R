...
# LAB 3: Bias/Variance and VC-dimension
...
## Let's do a Bias/Variance analysis first (notice that, without the green line, we still do not know which polynomial is 
...
for (i in 1:10)
{
  ...
  plot(data.frame(x, t), xlab=paste("Polynomial fit of degree 1, \ndata sample", i), ylab="f(x)",xlim=c(0,1),ylim=c(-1.25,1.25))
  curve(f, type="l", col="green", add=TRUE)
  polyfit <- lm(t ~ poly(x, 1, raw=TRUE))
  p <- polynom(coef(polyfit))
  curve(p, col="red", add=TRUE)
...
for (i in 1:10)
{
  ...
  plot(data.frame(x, t), xlab=paste("Polynomial fit of degree 6, \ndata sample", i), ylab="f(x)",xlim=c(0,1),ylim=c(-1.25,1.25))
  curve(f, type="l", col="green", add=TRUE)
...
## What are we going to do? How can we avoid overfitting/underfitting? (note that most often the real danger is in overfitting; 
## this is because many ML methods tend to be very flexible, i.e., they are able to represent complex models)
...
## Now let us use part of the data for validation (this is what cross-validation does, although in a more systematic way: 
## we will see this in a future lab session)

## We are going to generate the same plot as before, but this time we will be computing the prediction error on 
## the validation data, which we create as an independent (and larger) sample
...
t.val <- f(x.val) + rnorm(N.val, sd=0.2)      # generate the t according to a gaussian conditional distribution p(t|x)
...
## Inspect the results. Which degree yields the lowest error on validation data? There is a close difference between degrees 3 and 4; 
## typically that for degree 3 is a bit smaller
...
## The third method (using explicit complexity control, a.k.a. regularization) will actually be the topic of the next lab session, 
## so we defer this analysis for then
...
results <- matrix (nrow=9, ncol=4)
colnames(results) <- c("Degree","TR error","VC dim","bound")
degrees <- seq(9)
