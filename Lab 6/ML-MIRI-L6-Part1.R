...
# LAB 6: Clustering (k-means and E-M) (Part1)
...
D <- data.frame (x, color=c(rep("4", N1), rep("2", N2), rep("3", N3)))
...
# Clustering quality as measured by the Calinski-Harabasz index (as seen in class)
...
## plot the cluster centers
points(kmeans.5$centers,bg=seq(1:kmeans.5$ncenters)+1,cex=2,pch=21,col='black')
...
## Let's have a look at the unconditional density p(x)
...
# Exoplanet Data: exoplanets are planets outside our Solar System. They are almost always detected by indirect means, for instance, using the gravitational influence they exert on their central stars. Exoplanets are not at all like the nine local planets we know so well. Therefore a first step in the process of understanding the exoplanets might be to try to classify them with respect to their known properties.
# 
...
# This example shows the danger of treating data without any pre-processing; it also shows the inadequacy of C-H to assess different numbers of clusters: sometimes this number is not penalized enough
...
# The last thing we can do is to let the method decide everything (including the number of clusters); this is usually done using the BIC as a heuristic
