...
# Which is the target variable? where is it? how many different values? is it a 
# classification problem or a regression problem?

# answers: the target variable is located in column 1 and is called 'Assessment'; it has two possible values
...
# Perform a basic inspection of the dataset. Have a look at the minimum 
# and maximum values for each variable; find possible errors and abnormal 
# values (outliers); find possible missing values 
# (these are marked by '?', '99999', 'NA' and sometimes symply by '0'); 
# decide which variables should be continuous and which categorical; if there 
# are mixed types, we have three options: recode continuous to categorical, 
# recode categorical to continuous or leave them as they are. The thing is 
# that some ML methods feel more comfortable with one of these three options ...
...
# Make a decision on a sensible treatment for the missing values and apply it; it is wise 
# to write down the possible consequences of this decision and the alternatives that could 
# be considered in case the final results are not satisfactory
...
# Continuous variables have too many missing values, we can not eliminate them just like that: 
# we must devise a treatment for these missing values
...
# imputation by 1NN (nearest-neighbour): for every data example with a missing 'Income', 
# we look for the most similar one (according to the remaining variables) and copy its 'Income' value
...
# In this case we have decided to keep the original type and leave the decision for later, 
...
# Decide whether it can be sensible to derive new variables; we decide to extract two 
# new continuous and one new categorical variable (for the sake of illustration):
...
Credit.new <- data.frame(Assessment,YearsInJob,Housing,Deadline,
                         Age,MaritalStatus,Records,TypeOfJob,Expenses,
                         Income,Capital,ChargesOnCapital,AmountRequested,
                         MarketPrice,FinancingRatio,SavingCapacity,Dubious)
...
# we can perform graphical comparisons between some pairs of variables 
# (both categorical and continuous), using the plot(),  pairs() and identify() procedures

par(mfrow=c(1,2))
plot (AmountRequested, Capital, main = "Amount req. vs. Market price", 
      cex = .5, col = "dark red")

plot (log10(AmountRequested), log10(Capital+1), main = "Amount req. vs. Market price (better)", 
      cex = .5, col = "dark red")
...
# Most R functions for modelling accept factors mixed with numeric data; 
# the factors are silently recoded using a dummy binary code, so there is no need to handle this explicitly
...
Credit.new.CAT <- data.frame(Assessment,YearsInJob.CAT,Housing,Deadline.CAT,Age.CAT,
                             MaritalStatus,Records,TypeOfJob,Expenses.CAT,Income.CAT,
                             Capital.CAT,ChargesOnCapital.CAT,AmountRequested.CAT,
                             MarketPrice.CAT,FinancingRatio.CAT,SavingCapacity.CAT)
...
pairs(Credit.new[,c(2,5,9,16)], main = "Credit Scoring DataBase", col = (1:length(levels(
  Credit.new$Assessment)))[unclass(Credit.new$Assessment)])
...
varc <- list(YearsInJob, Deadline, Age, Expenses, Income, Capital, ChargesOnCapital, AmountRequested, 
             MarketPrice, FinancingRatio, SavingCapacity)
...
for (i in 1:ncon) 
{
  barplot (tapply(varc[[i]], Credit.new$Assessment, mean),main=paste("Means by",
                                                                     row.names(pvalcon)[i]), las=2, cex.names=0.75)
