...
# 4. perform a basic inspection of the dataset. Have a look at the minimum and maximum 
#    values for each variable; find possible errors and abnormal values (outliers); find 
#    possible missing values; decide which variables are continuous and which are categorical
# 5. make a decision on a sensible treatment for the missing values and apply it; 
#    WARNING: 'origin' is categorical and cannot be used for knn imputation, unless you make it 
#    binary temporarily
# 6. derive one new continuous variable: weight/horsepower; derive one new categorical variable: 
#    sports_car, satisfying horsepower > 1.2*mean(horsepower) AND acceleration < median(acceleration); 
#    do you think this new variable is helpful in predicting 'origin' ?
...
# 11. choose one of the continuous variables and recode it as categorical; choose one of the 
#    categorical variables and recode it as continuous
# 12. create a new dataframe that gathers everything and inspect it again; consider 'origin' as the 
#    target variable; perform a basic statistical analysis as indicated in SECTION 9
